...
class Loader:
    ...
    def __init__(self, conn, token, repo_user, repo_name, max_rows_load_issues, max_rows_load_pullrequests):
        logging.basicConfig(
            filename="../output/loaddata.log", level=logging.INFO)
        self.conn = conn
        self.token = token
    ...
    def _get_repository_id(self):
        ...
        cursor_conn.execute(
            sql, ["{}/{}".format(self.repo_user, self.repo_name)])
        id = 0
    ...
    def _load_contributors(self, gh, contributors):
        if contributors is not None:
            ...
            for contributor in contributors:
                ...
                args = (contributor_login_name, contributor.login,
                        contributor.url, contributor_followers, contributor_following)
                cursor_conn = self.conn.cursor()
                cursor_conn.execute(sql, args)
                ...
                self.conn.commit()
                self._load_repositories_contributors(
    ...
    def _load_issues(self, issues):
        ...
        for issue in issues:
            ...
            args = (self.repository_id, issue.user.login, issue.id,
                    issue.title, issue.created_at, issue.closed_at)
            cursor_conn.execute(sql, args)
            self.conn.commit()
    ...
    def _load_pull_requests(self, pull_requests):
        ...
        for pull_request in pull_requests:
            ...
            args = (self.repository_id, str(pull_request.id),  str(pull_request.issue(
            ).id), pull_request.created_at, pull_request.state, pull_request.closed_at, str(pull_request.user))
            cursor_conn.execute(sql, args)
            self.conn.commit()
    ...
    def _load_releases_data(self, insert_releasesdata_sql, standardize_releasesdata_sql):
        ...
        sql_insert_rawdata = "INSERT INTO ReleasesRawData(IdRelease, IdRepository, AuthorInfluencer, ClosedIssues, ClosedPullRequests, ClosedIssuesInfluencer, ClosedPullRequestsInfluencer, PrereleaseClass) SELECT IdRelease, IdRepository, AuthorInfluencer, ClosedIssues, ClosedPullRequests, ClosedIssuesInfluencer, ClosedPullRequestsInfluencer, PrereleaseClass FROM ReleasesData;"
    ...
    def _standardize_releasesdata(self, standardize_releasesdata_sql):
        ...
        sql_standardize = standardize_releasesdata_sql.format(
            self.repository_id, self.repository_id, self.repository_id, self.repository_id, self.repository_id)
        ...
        if cursor_fetch:
            ...
            print("{} ::        count_nrc={}, count_rc={}".format(
                datetime.today().strftime('%Y-%m-%d-%H:%M:%S'), count_nrc, count_rc))
            logging.info("{} ::         count_nrc={}, count_rc={}".format(
                datetime.today().strftime('%Y-%m-%d-%H:%M:%S'), count_nrc, count_rc))
        ...
        if (count_nrc > 0) and (count_rc > 0):
            if count_nrc != count_rc:
                ...
                print("{} ::        pre_cond_balacing={}, n_inserts={}".format(
                    datetime.today().strftime('%Y-%m-%d-%H:%M:%S'), pre_cond_balacing, n_inserts))
                logging.info("{} ::         pre_cond_balacing={}, n_inserts={}".format(
                    datetime.today().strftime('%Y-%m-%d-%H:%M:%S'), pre_cond_balacing, n_inserts))
            ...
            print("{} :: WARNING: The repository choosen doesn't have Releases nor Releases Candidates. Classification will not work!".format(
                datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))
            logging.info("{} :: WARNING: The repository choosen doesn't have Releases nor Releases Candidates. Classification will not work!".format(
                datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))

    def _truncate_previous_data(self, entity_name, field_name):
        cursor_delete = self.conn.cursor()
        # DELETE CASCADE expected
        cursor_delete.execute("PRAGMA foreign_keys = ON")
        sql_delete = "DELETE FROM {} WHERE {} = {}".format(
            entity_name, field_name, self.repository_id)
    ...
    def load(self, insert_releasesdata_sql, standardize_releasesdata_sql, load_type):
        ...
        if (load_type == 1) or (load_type == 2):
            if self.repository_id != 0:
                print("{} ::     Truncating previous repository data...".format(
                    datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))
                logging.info("{} ::     Truncating previous repository data...".format(
                    datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))
                self._truncate_previous_data("Releases", "IdRepository")
                self._truncate_previous_data("Repositories", "Id")

            print("{} ::     *) load_repository".format(
                datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))
            logging.info("{} ::     *) load_repository".format(
                datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))
            self._load_repository(repository)

            print("{} ::     *) load_contributors".format(
                datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))
            logging.info("{} ::     *) load_contributors".format(
                datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))
            contributors = repository.contributors()
            self._load_contributors(gh, contributors)

            print(
                "{} ::     *) load_releases".format(datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))
            logging.info(
                "{} ::     *) load_releases".format(datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))
        ...
        if (load_type == 1) or (load_type == 3):
            ...
            print(
                "{} ::     *) load_issues".format(datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))
            logging.info(
                "{} ::     *) load_issues".format(datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))
            issues = None
            if self.max_rows_load_issues > 0:
                issues = repository.issues(
                    state='closed', number=self.max_rows_load_issues)
            else:
        ...
        if (load_type == 1) or (load_type == 4):
            ...
            print("{} ::     *) load_pull_requests".format(
                datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))
            logging.info("{} ::     *) load_pull_requests".format(
                datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))
            pull_requests = None
            if self.max_rows_load_pullrequests > 0:
                pull_requests = repository.pull_requests(
                    state='closed', number=self.max_rows_load_pullrequests)
            else:
        ...
        if (load_type == 1) or (load_type == 5):
            ...
            print("{} ::     *) load_releases_data".format(
                datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))
            logging.info("{} ::     *) load_releases_data".format(
                datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))
            self._load_releases_data(
                insert_releasesdata_sql, standardize_releasesdata_sql)
